...
<script type="text/javascript">
	function deleteBoard(num) {
		...
		location.href = "${pageContext.request.contextPath}/free/delete.do?freeNum=" + num;
	}
	...
	function replyDelete(replyNum, userId) {
		...
		location.href = "${pageContext.request.contextPath}/free/replyDelete.do?${query}&freeNum=${dto.freeNum}&replyNum=" + replyNum;
	}
	
	function replyADD(freeNum) {
		...
		f.action = "${pageContext.request.contextPath}/free/replyADD.do?freeNum=" + freeNum + "&page=" + "${page}";
		f.submit();
	}
</script>
...
<body>
	...
	<div id="mainBody">
		...
		<main>
			...
			<tr style="border-bottom: 1px solid #cccccc;">
			  <td colspan="2" align="left" style="padding: 10px 5px;" valign="top" height="200">
			  	  <c:forEach var="img" items="${imgList}">
			  	  	<img src="${pageContext.request.contextPath}/uploads/free/${img.saveFileName}"/><br>
			  	  </c:forEach>
			      ${dto.content}
			   </td>
			</tr>
			...
			<tr height="35" style="border-bottom: 1px solid #cccccc;">
				...
				<c:if test="${not empty dto_pre}">
					<a href="${pageContext.request.contextPath}/free/article.do?${query}&num=${dto_pre.freeNum}">${dto_pre.subject}</a>
				</c:if>
			...
			<tr height="35" style="border-bottom: 1px solid #cccccc;">
				...
				<c:if test="${not empty dto_next}">
       				<a href="${pageContext.request.contextPath}/free/article.do?${query}&num=${dto_next.freeNum}">${dto_next.subject}</a>
    			</c:if>
			...
			<tr height="45">
			    ...
			    <c:if test="${sessionScope.member.userId == dto.userId}">
			          <button type="button" class="btn" onclick="javascript:location.href='${pageContext.request.contextPath}/free/update.do?page=${page}&freeNum=${dto.freeNum}';">수정</button>
			    </c:if>
			          <button type="button" class="btn" onclick="deleteBoard('${dto.freeNum}');">삭제</button>
			    </td>
			
			    <td align="right">
			        <button type="button" class="btn" onclick="javascript:location.href='${pageContext.request.contextPath}/free/list.do?${query}';">리스트</button>
			    </td>
			</tr>
			</table>
     ...
     <tr style="border-bottom: 1px solid #ccc;">
         ...
         <td>
         	<button type="button" class="btn" onclick="replyADD('${dto.freeNum}');">등록</button>
         </td>
     </tr>
     </table>
   		</form>
